...
function EditarPromocao() {
  ...
  const updateMut = useMutation({
    ...
    onError: (e) => toast.error((e as Error).message || t("edit.saveError")),
  });

  const deleteMut = useMutation({
    mutationFn: () => deletarPromocao(id),
    onSuccess: async () => {
      toast.success(t("edit.removed"));
      await qc.invalidateQueries({ queryKey: ["bootstrap"] });
      navigate({ to: "/hub" });
    },
    onError: (e) => toast.error((e as Error).message || t("edit.deleteError")),
  });
  ...
  return (
    <div className="px-6 lg:px-12 py-10 pt-20 lg:pt-14 max-w-7xl mx-auto">
      ...
      <header className="flex items-start justify-between gap-4 mb-10">
        <div>
          <h1 className="manual-comercial-gold text-4xl sm:text-5xl font-display font-black tracking-tight leading-tight pb-2 break-words">
            {promocao?.nome_interno || promocao?.nome || t("create.title")}
          </h1>
        </div>
        {promocao && canDelete && (
          <button
            onClick={() => {
              if (confirm(t("edit.deleteConfirm"))) deleteMut.mutate();
            }}
      ...
      {isLoading && <p className="text-white/40 text-sm uppercase tracking-widest">{t("common.loading")}</p>}
      {!isLoading && !promocao && (
