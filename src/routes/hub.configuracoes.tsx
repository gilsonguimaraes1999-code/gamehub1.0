...
const CAMPOS: { key: string; labelKey: string; type?: "text" | "textarea" }[] = [
  { key: "logoTexto", labelKey: "settings.logoText" },
  { key: "logoSubtexto", labelKey: "settings.logoSubtext" },
  { key: "loginTitulo", labelKey: "settings.loginTitle" },
  { key: "loginSubtitulo", labelKey: "settings.loginSubtitle" },
  { key: "rodapeLogin", labelKey: "settings.loginFooter" },
  { key: "sidebarTitle", labelKey: "settings.sidebarTitle" },
  { key: "sidebarSubtitle", labelKey: "settings.sidebarSubtitle" },
  { key: "dashboardTitle", labelKey: "settings.dashboardTitle" },
  { key: "dashboardSubtitle", labelKey: "settings.dashboardSubtitle", type: "textarea" },
  { key: "botaoNovaPromocao", labelKey: "settings.newPromotionButton" },
  { key: "placeholderPesquisa", labelKey: "settings.searchPlaceholder" },
];
...
function ConfiguracoesPage() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: async () => { toast.success(t("settings.interfaceSaved")); await qc.invalidateQueries({ queryKey: ["bootstrap"] }); },
    onError: (e) => toast.error((e as Error).message || t("settings.saveError")),
  });
  ...
  const handleLogo = async (file: File) => {
    ...
    try {
      ...
      setLogoUrl(url);
      toast.success(t("settings.logoSent"));
    } catch (e) {
  ...
  return (
    <div className="px-6 lg:px-12 py-10 pt-20 lg:pt-14 max-w-3xl mx-auto">
      ...
      <header className="flex items-start justify-between gap-4 mb-10">
        ...
        >
          <Save size={14} /> {saveMut.isPending ? t("common.saving") : t("common.save")}
        </button>}
      </header>
      ...
      {!canView ? (
        ...
        <p className="text-white/40 text-sm uppercase tracking-widest">{t("common.loading")}</p>
      ) : (
        <div className="space-y-8">
          {/* Logo */}
          <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
            <label className={labelCls}>{t("settings.logoImage")}</label>
            <div className="flex flex-wrap items-center gap-4">
              ...
              {canEdit && <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-3 rounded-xl border border-dashed border-white/20 text-white/70 hover:border-[#d4af37]/50 hover:text-white text-sm">
                <Upload size={16} /> {uploading ? t("common.uploading") : logoUrl ? t("settings.change") : t("settings.choose")}
                <input type="file" accept="image/*" hidden onChange={(e) => e.target.files?.[0] && handleLogo(e.target.files[0])} />
              </label>}
              <input value={logoUrl} onChange={(e) => setLogoUrl(e.target.value)} readOnly={!canEdit} placeholder={t("settings.imageUrl")} className={fieldCls + " flex-1 min-w-64"} />
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {CAMPOS.map((c) => (
              <div key={c.key} className={c.type === "textarea" ? "md:col-span-2" : ""}>
                <label className={labelCls}>{t(c.labelKey)}</label>
                {c.type === "textarea" ? (
                  <textarea
